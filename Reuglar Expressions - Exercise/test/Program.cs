namespace _03._The_Pianist
{
    ...
    internal class Program
    {
        ...
        {
            var collectionsPiece = new List<PianoKey>();
            FillCollections(collectionsPiece);
            OperationColections(collectionsPiece);
            PrintColections(collectionsPiece);
        }
        ...
        public static void OperationColections(List<PianoKey> colectionsPiece)
        {
            ...
            while ((comand = Console.ReadLine()) != "Stop")
            {
                ...
                if (comandArg[0] == "Add")
                {
                    CommandAddCollections(colectionsPiece, comand);
                }
        ...
        public static void CommandAddCollections(List<PianoKey> colectionsPiece, string comand)
        {
            string[] pieceArg = comand
                   .Split("|", StringSplitOptions.RemoveEmptyEntries);
        ...
        public static void FillCollections(List<PianoKey> collectionsPiece)
        {
            int collectionLengt = int.Parse(Console.ReadLine());

            for (int i = 0; i < collectionLengt; i++)
            {
                string[] pieceArg = Console.ReadLine()
                    .Split("|", StringSplitOptions.RemoveEmptyEntries);
                var piece = new PianoKey(pieceArg[0], pieceArg[1], pieceArg[2]);

                collectionsPiece.Add(piece);
